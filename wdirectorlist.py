...
for year in winners:
  for i in range(len(winners[year])):
    if winners[year][i] not in win_count_dict:
      win_count_dict[winners[year][i]] = 1
    else: #se winners[year][i] in win_count_dict:
      win_count_dict[winners[year][i]] += 1
      continue
for name, num_oscar in win_count_dict.items():
  if num_oscar > highest_count:
    highest_count = num_oscar
    most_win_director.clear()
    most_win_director.append(name)
  elif num_oscar == highest_count:
    most_win_director.append(name)
  else:
    continue
    #print(highest_count)
